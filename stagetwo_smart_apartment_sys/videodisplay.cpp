...
void videodisplay::setMyName(QString name)
{
    myname = name;
}
...
void videodisplay::on_commandLinkButton_clicked()
{
//    pd.close();
    ...
    emit userw->sendUserName(myname);
...
//    stop_device(&pd);
    timer->stop();
    disconnect(timer,&QTimer::timeout,this,&videodisplay::flushButff);
//    timer->setInterval(20);
    stop_capturing(&pd);
    stop_device(&pd);
    close_device(&pd);
